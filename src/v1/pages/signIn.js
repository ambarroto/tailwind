...
import { removeFormError, resetFormError, resetLoginForm, setLoginForm, userLogin } from '../store/mainSlice';
import { ErrorMessage, Input, Label } from './admin/components/Form';
...
export default function SignIn() {
    ...
    const form = useSelector((state) => state.main.loginForm)
    const headers = useRequestHeaders()
    const login = useSelector((state) => state.main.login)

    const inputForm = (event, value = '') => {
        const name = event.target.name
        dispatch(setLoginForm({ field: name, value: value }))
    ...
    useEffect(() => {
        if (login) navigate('/v1/admin', { replace: true })
    }, [login, navigate])

    useEffect(() => {
        if (accessToken) navigate('/v1/admin', { replace: true })
    }, [accessToken, navigate])

    const handleSubmit = async (event) => {
        event.preventDefault();
        dispatch(userLogin({headers, form}))
    };

    return (
        <form onSubmit={ (e) => handleSubmit(e) }>
            <div className="h-full sm:h-screen bg-gradient-to-tl from-green-400 to-indigo-900 w-full py-16 px-4">
                <div className="flex flex-col items-center justify-center">
                    ...
                    <div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-16">
                        <p tabIndex={0} aria-label="Login to this system" className="text-2xl font-extrabold leading-6 text-gray-800">
                        ...
                        <div>
                            <Label for={'username'} label={'Email/Username'} error={ formError.username } />
                            <Input id={'username'} error={ formError.username } name={ 'username' } onInput={ (e) => inputForm(e, e.target.value) } />
                            { formError.username && <ErrorMessage message={ formError.username.msg } /> }
                        </div>
                        <div className="mt-6  w-full">
                            <Label for={'password'} label={'Password'} error={ formError.password } />
                            <Input type='password' id={'password'} error={ formError.password } name={ 'password' } onInput={ (e) => inputForm(e, e.target.value) } />
                            { formError.password && <ErrorMessage message={ formError.password.msg } /> }
                        </div>
                        <div className="mt-8">
                            <button aria-label="sign in" type='submit' className="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 text-sm font-semibold leading-none text-white focus:outline-none bg-indigo-700 border rounded hover:bg-indigo-600 py-4 w-full">
                                Sign In
                            </button>
                        </div>
                    </div>
